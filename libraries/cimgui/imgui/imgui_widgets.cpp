...
void ImGui::TextAlignedV(float align_x, float size_x, const char* fmt, va_list args)
{
    ...
    RenderTextEllipsis(window->DrawList, pos, pos_max, 0, pos_max.x, text, text_end, &text_size);
...
void ImGui::SeparatorTextEx(ImGuiID id, const char* label, const char* label_end, float extra_w)
{
    ...
    if (label_size.x > 0.0f)
    {
        ...
        RenderTextEllipsis(window->DrawList, label_pos, ImVec2(bb.Max.x, bb.Max.y + style.ItemSpacing.y), 0, bb.Max.x, label, label_end, &label_size);
    }
    else
    {
        if (g.LogEnabled)
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id, bool is_contents_visible, bool* out_just_closed, bool* out_text_clipped)
{
    ...
    LogSetNextTextDecoration("/", "\\");
    RenderTextEllipsis(draw_list, text_ellipsis_clip_bb.Min, text_ellipsis_clip_bb.Max, 0, ellipsis_max_x, label, NULL, &label_size);
