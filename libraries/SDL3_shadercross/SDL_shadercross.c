...
/* DXCompiler, loaded dynamically (no import library is shipped for dxcompiler.dll). */
...
static pfn_DxcCreateInstance SDL_DxcCreateInstance = NULL;

static bool SDL_ShaderCross_INTERNAL_LoadDXC(void)
{
    ...
    SDL_DxcCreateInstance = (pfn_DxcCreateInstance)SDL_LoadFunction(dxcompiler_dll, "DxcCreateInstance");
    return SDL_DxcCreateInstance != NULL;
}

#endif /* SDL_SHADERCROSS_DXC */

/* Whether DXIL output is actually possible at runtime. */
static bool SDL_ShaderCross_INTERNAL_DXCAvailable(void)
{
#ifdef SDL_SHADERCROSS_DXC
    return SDL_ShaderCross_INTERNAL_LoadDXC();
#else
    return false;
#endif
}
    ...
    size_t *size) // filled in with number of bytes of returned buffer
{
    ...
    if (!SDL_ShaderCross_INTERNAL_LoadDXC()) {
        SDL_SetError("%s", "Could not load " DXCOMPILER_DLL "!");
        return NULL;
    }
    ...
    // The HLSL fed to DXC here is machine-generated by SPIRV-Cross; lint warnings about
    // its style (e.g. `1 << n` literal-shift ambiguity) are pure noise for the user.
    args[argCount++] = (LPCWSTR)L"-Wno-ambig-lit-shift";
    ...
    void *metadata)
{
    ...
    if (shader_formats & SDL_GPU_SHADERFORMAT_SPIRV) {
        ...
        // Storage buffers cannot be expressed in the FXC/DXBC (SM 5.0) path, and a
        // pipeline cannot mix DXBC and DXIL stages -- so when DXC is available prefer
        // DXIL (SM 6.0) for all graphics shaders, keeping every pipeline's stage pair
        // homogeneous. Without DXC, fall back to the long-standing DXBC behavior
        // (shaders that require storage buffers then fail to create, and callers can
        // degrade gracefully).
        if ((shader_formats & SDL_GPU_SHADERFORMAT_DXIL) && SDL_ShaderCross_INTERNAL_DXCAvailable()) {
            format = SDL_GPU_SHADERFORMAT_DXIL;
        } else if ((shader_formats & SDL_GPU_SHADERFORMAT_DXBC) && SDL_D3DCompile != NULL) {
...
SDL_GPUShaderFormat SDL_ShaderCross_GetSPIRVShaderFormats(void)
{
    ...
    if (SDL_ShaderCross_INTERNAL_DXCAvailable()) {
