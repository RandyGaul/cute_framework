...
# The harness executables, cute-h264-test and cute-h264-decode from tools/CMakeLists.txt.
TEST = os.environ.get('H264TEST', './build/cute-h264-test.exe')
DEC = os.environ.get('H264DEC', './build/cute-h264-decode.exe')
    ...
    for cab in (0,1):
        for (w,h) in SIZES:
            for qp in QPS:
                ...
                subprocess.run([TEST,str(w),str(h),'8',str(qp),'0'],capture_output=True,env=e)
                ...
                o=subprocess.run([DEC,'out.264','ours.yuv'],capture_output=True,text=True)
