...
{
	make_app("Pivot", 0, 0, 0, 640, 480, CF_APP_OPTIONS_WINDOW_POS_CENTERED_BIT, argv[0]);
	...
	CF_Sprite animated_box_with_pivots = cf_make_sprite("/pivot.ase");

	while (app_is_running()) {
		app_update();
		draw_scale(4,4);

		sprite_update(animated_box_with_pivots);
		sprite_draw(animated_box_with_pivots);
