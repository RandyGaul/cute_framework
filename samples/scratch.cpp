...
int main(int argc, const char** argv)
{
	...
	while (app_is_running()) {
		...
		cf_draw_text("ProggyClean", "Some bigger and blue text.", V2(-100,-30));
		cf_draw_pop_color();
		cf_draw_pop_font_size();

		cf_draw_push_font_size(13 * 5);
		cf_draw_push_font_blur(10);
		cf_draw_text("ProggyClean", "glowing~", V2(-150-10,-90+10));
		cf_draw_pop_font_blur();
		cf_draw_text("ProggyClean", "glowing~", V2(-150,-90));
		cf_draw_pop_font_size();

		cf_draw_push_font_size(13 * 10);
		cf_draw_push_font_blur(10);
		cf_draw_push_color(color_black());
		cf_draw_text("ProggyClean", "shadow", V2(-150-10-2.5f,-200+5));
		cf_draw_pop_color();
		cf_draw_pop_font_blur();
		cf_draw_text("ProggyClean", "shadow", V2(-150,-200));
		cf_draw_pop_font_size();

		String draws;
		draws.fmt("Draw calls: %d", draw_calls);
		cf_draw_push_font_size(13);
		cf_draw_text("ProggyClean", draws.c_str(), V2(-w/2 + 10,-h/2 + 10));
		cf_draw_pop_font_size();
