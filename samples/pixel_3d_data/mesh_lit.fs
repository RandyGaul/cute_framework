...
//
// This block only declares what the fragment stage uses -- the vertex stage's matrices live in
// its own block. Per-stage blocks with different members are fine on every backend (GLES gets
// stage-prefixed block names under the hood).
...
// The spot's projection is perspective, so its bias rides the near plane: with znear pushed
// out to meet the scene (see the sample's spot_proj comment) depth resolution is ~10x better
// than a 0.1 near plane, and the bias tightens to match -- loose biases here open a visible
// gap between the boxes and their spot shadows.
float spot_shadow_factor(vec3 n, vec3 to_spot)
{
	if (u_material.w <= 0.0) return 1.0;
	float ndl = max(dot(n, to_spot), 0.0);
	return pcf(u_spot_shadow_map, v_spot_pos, mix(0.002, 0.0005, ndl), u_material.w);
}
