...
CF_TEST_CASE(test_circular_buffer_basic, "Typical use-case example, push and pull some data.");
int test_circular_buffer_basic()
{
	CF_CircularBuffer buffer = cf_make_circular_buffer(1024);
	CF_TEST_CHECK_POINTER(buffer.data);

	const char* the_data = "Here's some data.";
	int the_data_size = (int)CF_STRLEN(the_data) + 1;

	CF_TEST_CHECK(cf_circular_buffer_push(&buffer, the_data, the_data_size));
	void* pull_data = malloc(the_data_size);
	CF_TEST_CHECK(cf_circular_buffer_pull(&buffer, pull_data, the_data_size));
	CF_TEST_ASSERT(memcmp(the_data, pull_data, the_data_size) == 0);
	free(pull_data);
...
CF_TEST_CASE(test_circular_buffer_fill_up_and_empty, "Fill up the buffer and empty it a few times.");
int test_circular_buffer_fill_up_and_empty()
{
	...
	for (int iters = 0; iters < 5; ++iters) {
		for (int i = 0; i < bytes; ++i) {
			...
			CF_TEST_CHECK(cf_circular_buffer_push(&buffer, &byte, 1));
		}

		for (int i = 0; i < bytes; ++i) {
			uint8_t byte;
			CF_TEST_CHECK(cf_circular_buffer_pull(&buffer, &byte, 1));
			CF_TEST_ASSERT(byte == i);
		}
	}
...
CF_TEST_CASE(test_circular_buffer_overflow, "Attempt to push too much data to the buffer.");
...
CF_TEST_CASE(test_circular_buffer_underflow, "Attempt to pull too many bytes from the buffer.");
int test_circular_buffer_underflow()
{
	...
	uint8_t bytes_11[11];
	CF_TEST_CHECK(!cf_circular_buffer_pull(&buffer, bytes_11, 11));
...
int test_CircularBufferwo_threads_pull(void* data)
{
	...
	while (iters) {
		...
		if (result) continue;
		CF_TEST_ASSERT(expected_val == got_val);
		++expected_val;
		--iters;
	}
...
CF_TEST_CASE(test_CircularBufferwo_threads, "Run a producer and a consumer thread, and validate input/output of the buffer.");
