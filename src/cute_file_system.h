...
extern CUTE_API error_t CUTE_CALL file_system_seek(file_t* file, uint64_t position);
...
extern CUTE_API error_t CUTE_CALL file_system_read_entire_file_to_memory(const char* virtual_path, void** data_ptr, uint64_t* size = NULL, void* user_allocator_context = NULL);
extern CUTE_API error_t CUTE_CALL file_system_write_entire_buffer_to_file(const char* virtual_path, const void* data, uint64_t size);
