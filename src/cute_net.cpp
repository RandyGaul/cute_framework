...
static CUTE_INLINE uint64_t s_client_read_sequence_number(void* packet)
{
	return 0;
}
...
{
	uint64_t sequence = s_client_read_sequence_number(packet);
	crypto_nonce_t nonce = s_nonce_from_sequence(sequence);
	...
	if (client->state == CLIENT_STATE_CONNECTING) {
		// Initially clients use asymmetric encryption to verify the identity of the server.
		if (crypto_decrypt_asymmetric(&client->socket.endpoint.public_key, &client->session_key, (uint8_t*)packet, size, &nonce) < 0) {
			// Forged packet!
			return NULL;
		}
	} else {
		// Majority of traffic uses the symmetric key, generated by the client, used as a shared-secret between
		// *only* the client and the server.
		if (crypto_decrypt(&client->session_key, (uint8_t*)packet, size, &nonce) < 0) {
			// Forged packet!
			return NULL;
		}
	}

	*type = s_client_packet_type(packet);
	return packet;
}
...
static void s_client_receive_packets(client_t* client)
{
	...
	{
		endpoint_t from;
		int bytes_read = s_socket_receive(&client->socket, &from, buffer, sizeof(buffer));
		if (!bytes_read) break;

		packet_type_t type;
		void* packet = s_client_decrypt_packet(client, buffer, bytes_read, &type);
		if (!packet) {
