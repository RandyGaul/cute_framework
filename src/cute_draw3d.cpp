...
{
	// Mesh commands still sitting in a layer queue (recorded but never rendered) own their payloads.
	s_draw->clear_queues();
...
void cf_draw3d_prepare_uploads(int layer_lo, int layer_hi)
{
	...
	for (int q = 0; q < s_draw->layer_queues.count(); ++q) {
		CF_DrawLayer* lq = s_draw->layer_queues[q];
		if (lq->layer < layer_lo) continue;
		if (lq->layer > layer_hi) break; // Queues are kept in layer order.
		for (int i = 0; i < lq->cmds.count(); ++i) {
			CF_MeshCmd3d* mc = lq->cmds[i].mesh3d;
			if (!mc) continue;
...
// private set of layer queues and move into the list at cf_draw_list_end); the hooks below add the 3d
