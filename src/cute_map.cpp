...
// https://github.com/mattiasgustavsson/libs/blob/main/map.h
...
void* cf_map_make_impl(int key_size, int item_size, int capacity)
{
	...
	// This is critical to support return-by-value polymorphism in the C macro API for `mget` and `mfind`.
	// We also "pass" in values to `madd` through this space.
...
void cf_map_free_impl(CF_Hhdr* table)
{
	if (!table) return;
...
void* cf_map_insert_impl2(CF_Hhdr* table, const void* key, const void* item)
{
...
void* cf_map_insert_impl3(CF_Hhdr* table, const void* key)
{
	return cf_map_insert_impl2(table, key, table->hidden_item);
}

void* cf_map_insert_impl(CF_Hhdr* table, uint64_t key)
{
	return cf_map_insert_impl2(table, &key, table->hidden_item);
}

void cf_map_remove_impl2(CF_Hhdr* table, const void* key)
{
	uint32_t hash = (uint32_t)fnv1a(key, table->key_size);
...
void cf_map_remove_impl(CF_Hhdr* table, uint64_t key)
{
	cf_map_remove_impl2(table, &key);
}

void cf_map_clear_impl(CF_Hhdr* table)
{
	table->count = 0;
	for (int i = 0; i < table->slot_capacity; ++i) {
...
int cf_map_find_impl2(const CF_Hhdr* table, const void* key)
{
	int slot = s_find_slot(table, (uint32_t)fnv1a(key, table->key_size), key);
	if (slot < 0) {
		// We will be "returning" a zero'd out item through `mget` with this
		// hidden item.
...
int cf_map_find_impl(const CF_Hhdr* table, uint64_t key)
{
	return cf_map_find_impl2(table, &key);
}

bool cf_map_has_impl(CF_Hhdr* table, uint64_t key)
{
	return !!cf_map_find_impl(table, key);
}

int cf_map_count_impl(const CF_Hhdr* table)
{
	return table->count;
}

void* cf_map_items_impl(const CF_Hhdr* table)
{
	return table->items_data;
}

void* cf_map_keys_impl(const CF_Hhdr* table)
{
	return table->items_key;
}

void cf_map_swap_impl(CF_Hhdr* table, int index_a, int index_b)
{
