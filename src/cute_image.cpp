...
cf_error_t image_load_png(const char* path, image_t* img, void* user_allocator_context)
{
	void* data;
	size_t sz;
	cf_error_t err = file_system_read_entire_file_to_memory(path, &data, &sz, user_allocator_context);
...
cf_error_t image_load_png_mem(const void* data, int size, image_t* img, void* user_allocator_context)
{
...
cf_error_t image_load_png_wh(const void* data, int size, int* w, int* h)
{
...
cf_error_t image_load_png_indexed(const char* path, image_indexed_t* img, void* user_allocator_context)
{
	void* data;
	size_t sz;
	cf_error_t err = file_system_read_entire_file_to_memory(path, &data, &sz, user_allocator_context);
...
cf_error_t image_load_png_mem_indexed(const void* data, int size, image_indexed_t* img)
{
