...
CUTE_INLINE error_t image_load_png(const char* virtual_path, image_t* img, void* user_allocator_context = NULL) { return cf_image_load_png(virtual_path, img, user_allocator_context); }
CUTE_INLINE error_t image_load_png_mem(const void* data, int size, image_t* img, void* user_allocator_context = NULL) { return cf_image_load_png_mem(data, size, img, user_allocator_context); }
CUTE_INLINE error_t image_load_png_wh(const void* data, int size, int* w, int* h) { return cf_image_load_png_wh(data, size, w, h); }
CUTE_INLINE void image_free(image_t* img) { cf_image_free(img); }

CUTE_INLINE error_t image_load_png_indexed(const char* virtual_path, image_indexed_t* img, void* user_allocator_context = NULL) { return cf_image_load_png_indexed(virtual_path, img, user_allocator_context); }
CUTE_INLINE error_t image_load_png_mem_indexed(const void* data, int size, image_indexed_t* img) { return cf_image_load_png_mem_indexed(data, size, img); }
