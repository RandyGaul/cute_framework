 ...
 *            const cf_https_response_t* response = cf_https_response(https);
...
struct cf_https_string_t
{
	const char* ptr;
	size_t len;
};

struct cf_https_header_t
{
	cf_https_string_t name;
	cf_https_string_t content;
};

enum cf_transfer_encoding_t
{
 ...
 */
CUTE_API const cf_https_response_t* CUTE_CALL cf_https_response(cf_https_t* https);
