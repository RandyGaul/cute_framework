...
static color_t s_color = make_color(0.0f, 0.0f, 0.0f, 0.25f);
...
void light_system_init()
{
	...
	s_buf = triple_buffer_make(sizeof(v2) * 1000, sizeof(v2));
...
void light_system_update(app_t* app, float dt, void* udata, Transform* transforms, Light* lights, int entity_count)
{
	for (int i = 0; i < entity_count; ++i) {
		...
		const float r0 = 0.5f;
		coroutine_t* co = &light->co;
		COROUTINE_START(co);
		light->radius_delta = 0.0f + rnd_next_range(&s_rnd, -r0, r0);
		COROUTINE_PAUSE(co, rnd_next_range(&s_rnd, t0, t1), dt);
		light->radius_delta = 1.0f + rnd_next_range(&s_rnd, -r0, r0);
		COROUTINE_PAUSE(co, rnd_next_range(&s_rnd, t0, t1), dt);
		light->radius_delta = 2.0f + rnd_next_range(&s_rnd, -r0, r0);
		COROUTINE_PAUSE(co, rnd_next_range(&s_rnd, t0, t1), dt);
		light->radius_delta = 3.0f + rnd_next_range(&s_rnd, -r0, r0);
		COROUTINE_PAUSE(co, rnd_next_range(&s_rnd, t0, t1), dt);
		light->radius_delta = 2.0f + rnd_next_range(&s_rnd, -r0, r0);
		COROUTINE_PAUSE(co, rnd_next_range(&s_rnd, t0, t1), dt);
		light->radius_delta = 1.0f + rnd_next_range(&s_rnd, -r0, r0);
...
static void s_do_lights(array<v2>* verts)
{
	...
	if (verts->count()) {
		...
		s_uniforms(matrix_ortho_2d(320, 240, 0, 0));
...
{
	s_do_lights(&s_verts0);
	s_do_lights(&s_verts1);
}
