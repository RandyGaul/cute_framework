...
The main two functions for gathering keyboard input are [`cf_key_down`](../input/function/cf_key_down.md) and [`cf_key_just_pressed`](../input/function/cf_key_just_pressed.md).
...
You may also check if any alt, shift, or control key was pressed (either left or right) with [`cf_key_alt`](../input/function/cf_key_alt.md), [`cf_key_shift`](../input/function/cf_key_shift.md), and [`cf_key_ctrl`](../input/function/cf_key_ctrl.md).
...
Double clicks (both held and just-pressed) are available with [`cf_mouse_double_click_held`](../input/function/cf_mouse_double_click_held.md) and [`cf_mouse_double_clicked`](../input/function/cf_mouse_double_clicked.md).

The mouse coordinates can be fetched with [`cf_mouse_x`](../input/function/cf_mouse_x.md) and [`cf_mouse_y`](../input/function/cf_mouse_y.md). Each function returns an integer component. (0, 0) is the top-left of the screen, with the y-axis pointing downwards. It may be a little annoying to get mouse coordinates like this, since the default graphics coordinate system has (0, 0) at the center of the screen with the y-axis pointing upwards.

## Touch

Touch inputs come in the form of touch events ([`CF_Touch`](../input/struct/cf_touch.md)). Each touch event has a unique 64-bit identifier. Usually touch events are fairly short-lived, so it's up to you to note their unique ID's from one frame to another, and notice when a particular touch event dissappears.
...
To get list of all live touch events call [`cf_touch_get_all`](../input/function/cf_touch_get_all.md). This function fills in a pointer you can use to loop over all the touch events.
...
We have access to the operating system's IME through [`CF_ImeComposition`](../input/struct/cf_imecomposition.md) and friends. First, IME support must be enabled with [`cf_input_enable_ime`](../input/function/cf_input_enable_ime.md). From there the IME text can be viewed with [`cf_input_get_ime_composition`](../input/function/cf_input_get_ime_composition.md).

You can check for keyboard support in the IME with [`cf_input_has_ime_keyboard_support`](../input/function/cf_input_has_ime_keyboard_support.md), and know if the operating system is currently showing an IME keyboard on screen with [`cf_input_is_ime_keyboard_shown`](../input/function/cf_input_is_ime_keyboard_shown.md).

To tell the operating system where the IME should actually be on screen (as in, where the input rect is located) you can call [`cf_input_set_ime_rect`](../input/function/cf_input_set_ime_rect.md).
